...
const params = require("../../data/earlyVoting/CA/scrapeParams");
...
function geocodeAddress(county, address) {
  try {
    const knownLocations = require("../../data/earlyVoting/CA/knownLocations/" +
      county +
      ".json");
    var place = address
      .trim()
    ...
    if (loc) return loc;
      ...
      console.error("Unknown location in", county, "county CA:", address);
 ...
 * @param {*} stateid - must be CA
 * @param {*} locid - must be a valid county in CA
...
locationData = exports.locationData = (stateid, locid, amend) => {
  try {
    ...
    // earlier logic.
    if (stateid !== "CA") throw { ok: false, message: "Only GA and CA are supported" };
      ...
      .then((data) => {
        ...
        try {
          var inner = document.querySelector(
            "#divDataTable > div > #divDefault > #VoterCenterTable_wrapper > div > div > table > tbody"
          );
          if (inner) {
            var rows = inner.rows;
            var address = "";
...
exports.locations = (req, res) => {
  try {
    ...
    // Only GA in the module. If its not GA then probably there is an error in
    // earlier logic.
    if (stateid !== "CA") return res.status(404).send();
